...
rup_passed = False
for i in range(3):
  rue = float(open("closedEnsemble%dMDC2024a/rue" % i).readline())
  rup = float(open("closedEnsemble%dMDC2024a/rup" % i).readline())
...
if not rue_passed:
  ...
  fout = open("closedEnsemble%dMDC2024a/rue" % whichensemble,"w")
  fout.write("%e\n" % rue)
  fout.close()
...
if not rup_passed:
  ...
  fout = open("closedEnsemble%dMDC2024a/rup" % whichensemble,"w")
  fout.write("%e\n" % rup)
  fout.close()
